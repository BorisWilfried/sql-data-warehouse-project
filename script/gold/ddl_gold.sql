...
Script purpose :
	...
	Each View performs transformation and comnine data from the silver layer to produce a cleaned, 
...
SELECT 
  ...
  CASE 
		WHEN ci.cst_gndr != 'unknown' THEN ci.cst_gndr   -- CRM is Master for gender Info
		ELSE COALESCE(ca.gen,'unknown')
	END AS gender,
  ca.bdate AS birthdate,
  ci.cst_create_date AS create_date
FROM silver.crm_cust_info  ci
LEFT JOIN silver.erp_cusr_az12 ca
...
ON p.cat_id = c.id
WHERE prd_end_dt IS NULL     -- Filter out all historical Data (end_dt)
